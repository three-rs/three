...
fn main() {
    ...
    cam.transform_mut().position = [0.0, 100.0, 200.0].into();
    win.scene.add(&cam);
    ...
    let q = cgmath::Quaternion::from_angle_x(-cgmath::Rad::turn_div_4());
    let v: [f32; 3] = q.v.into();
    sea.transform_mut().set_all([0.0, -600.0, 0.0].into(),
                                mint::Quaternion { s: q.s, v: v.into() },
                                1.0);
    win.scene.add(&sea);

    let mut sky = sky::Sky::new(&mut rng, &mut win.factory);
    sky.group.transform_mut().position.y = -600.0;
    win.scene.add(&sky.group);

    let mut airplane = plane::AirPlane::new(&mut win.factory);
    airplane.group.transform_mut().scale = 0.25;
    airplane.group.transform_mut().position = [0.0, 100.0, 0.0].into();
